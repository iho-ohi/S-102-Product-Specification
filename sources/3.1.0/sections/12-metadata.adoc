...
In the following sections <<subsec-s100_exchangecatalogue>> and <<subsec-s100_datasetdiscoverymetadata>>, attribute multiplicities that differ from the <<iho-s100,part=17>> are indicated in bold type.
...
.Components and associated metadata for the {series}-{docnumber} exchange set (<<iho-s100,part=17,figure=2>> with items not used by {series}-{docnumber} omitted)
...
.Relationship between exchange catalogue, discovery metadata, and dataset (<<iho-s100,part=17,figure=6>> with items not used by {series}-{docnumber} omitted)
...
.{series}-{docnumber} Exchange Set Class Details (<<iho-s100,part=17,figure=7>> with items not used by {series}-{docnumber} omitted)
image::../images/figure-s102-exchange-set-class-details.png[]

[%portrait]
<<<

The following clauses define the mandatory and optional metadata needed for {series}-{docnumber}. In some cases, the metadata may be repeated in a national language. If this is the case it is noted in the Remarks column.
...
*Multiplicity is not adjusted, so that multiple different product specifications can still be included within a single exchange set. Otherwise, product specifications that support supportFileDiscoveryMetadata could not be included.*
...
[[subsec-s100_datasetdiscoverymetadata]]
...
*Format: file:/{series}-{docnumber}/DATASET_FILES/<dsname>* +
...
*For example, _{docnumber}DE00KD54.H5_* +
See note following <<iho-s100,part=17,table=S100_DatasetDiscoveryMetadata>> +
...
|Brand new dataset
|1
|No data has previously been produced for this area or there is a significant change from the existing, for example, coverage or extent.
...
*The corresponding Idx-number of the IHO Registry for {series}-{docnumber} Ed. {edition} is {iho-registry-ps-idx}.*
...
|-
|-
|*{series}-{docnumber} should use _category3_ or _category4_, subject to the guidance provided in <<subsec-s100-compliancy-category>>.*
...
The class PT_Locale is defined in <<iso-19115-1>>. LanguageCode, CountryCode, and MD_CharacterSetCode are ISO codelists which are defined in a codelists file which is part of the S-100 Edition {s100editionreference} schema distribution.
