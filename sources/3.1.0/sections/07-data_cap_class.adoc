...
The DCEG describes how data describing the real world should be captured using the types defined in the {series}-{docnumber} Feature Catalogue. The DCEG is located at <<annex-data-classification-and-encoding-guide>>.

A number of sounding techniques are used to capture bathymetric data. It is permitted, but not required, to include data acquisition information in the metadata of an {series}-{docnumber} Bathymetric Surface product. The metadata class S102_AcquisitionMetadata has been defined, but the information elements to populate this metadata class should be identified in a national profile of {series}-{docnumber}.

[[qualityAndSourceMetadata]]
=== Quality and source metadata

Quality and source metadata in {series}-{docnumber} are intended to enable and support future navigation software to appropriately auto-generate and attribute cartographic features such as custom depth contours and soundings from {series}-{docnumber} products, all while minimally impacting the overall file size of the product.
...
Quality and source metadata in {series}-{docnumber} are based on S-101 quality attributes, with significant augmentations and omissions described below. The quality and source metadata support a threefold purpose:

. Support S-101-defined attribution of auto-generated vector depth areas, depth contours, and soundings created directly from the {series}-{docnumber} dataset.
