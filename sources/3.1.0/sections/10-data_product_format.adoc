...
=== Introduction
The {series}-{docnumber} data set must be encoded using the Hierarchical Data Format standard, Version 5 (HDF5).
...
The key idea behind the {series}-{docnumber} product structure is that each coverage is a feature. Each of these features is co-located with the others. Therefore, they share the same spatial metadata, and each is required to correctly interpret the others.
...
image::../images/figure-outline-of-the-generic-data-file-structure.png[UML diagram depicting the generic data file structure as described between this figure and the table Overview of {series}-{docnumber} Data Product]
...
*Level 4*:: This level contains the actual data for each feature. In {series}-{docnumber} *BathymetryCoverage* and *QualityOfBathymetryCoverage* each use the *ValuesGroup* to define the content. The other groups at this level are not used.

In <<tab-overview-of-s102-data-product>> below, levels refer to HDF5 structuring (see <<fig-outline-of-the-generic-data-file-structure>>). Naming in each box below the header line is as follows: Generic name; S-100 or {series}-{docnumber} name, or nothing if none; and (_HDF5 type_) group, attribute or attribute list, or dataset. <<fig-hierarchy-of-s102-data-product>> depicts the same structure using a graphical representation.


[[tab-overview-of-s102-data-product]]
.Overview of {series}-{docnumber} Data Product
...
Example: INT.IHO.{series}-{docnumber}.{edition}
...
^| *0*
<| String
| The use of ISO metadata +
(per <<iho-s100,part=10c,clause=12>> & <<iho-s100,part=8>>) +
is not permitted in {series}-{docnumber}. +
The multiplicity is therefore set to 0. +
This is done in accordance with <<subsec-application-schema>> and <<sec-exchangeset>>.
...
[NOTE]
====
The _productIdentifier_ ("{series}-{docnumber}") and _version_ fields (X.X.X) of S100_ProductSpecification must be used.
====

[[note2]]
[NOTE]
====
The value _horizontalCRS_ specifies the horizontal Coordinate Reference System. At the time of writing, S-100 does not yet provide a mechanism for this value's definition within HDF5 encoding (such as an enumeration of horizontal CRSs). Consequently, this configuration causes a deviation from S-100. The horizontal datum is implicitly defined by this CRS because each horizontal CRS consists of a coordinate system and a datum. {series}-{docnumber} does not use "user defined" CRS as mentioned in <<iho-s100,part=10c,table=6>>.
====
...
.Contents of the BathymetryCoverage and QualityOfBathymetryCoverage arrays
...
|depth
|uncertainty
|featureAttributeTableKey

|name
|Long name as in Feature Catalogue
|depth
|uncertainty
|Feature Attribute Table Key
...
row *vertical datum* +
and <<mvdvdr,Multiple Vertical Datum>> +
...
| The only allowed value is 1: s100VerticalDatum +
(see <<iho-s100,part=10c,table=23>>). +
see <<mvdvdr,Multiple Vertical Datum>> +
...
| 1
| String
| Because {series}-{docnumber} specifies survey dates elsewhere in its structure, this value should always be the fill value: +
_00010101T000000Z_
...
The QualityOfBathymetryCoverage container group has the same metadata attributes as BathymetryCoverage container group (see <<tab-attributes-of-bathymetrycoverage-feature-container-group>>). The values of the attributes must also be the same as the BathymetryCoverage container group. Exceptions are the attribute dataCodingFormat, which must have the value ‘9’, and numInstances, which always has the value ‘1’. The use of multiple BathymetryCoverage Feature Instance groups (different Vertical Datums) does not affect the multiplicity of the QualityOfBathymetryCoverage, which remains 0 to 1. This means that the different BathymetryCoverage Feature Instance groups share a common QualityOfBathymetryCoverage.
...
//All optional except featureAttributeTableKey. Producers should not add other fields. (RM comment 23Jan2023) (DROH updated 25Aug2026)
...
[NOTE]
====
_surveyDateRange.dateStart_ and _surveyDateRange.dateEnd_ will use the following HDF5 encoding (fixed length string per <<iho-s100,part=10c,table=1>>):
[source,c]
----
H5T_STRING {
    STRSIZE 8;
...
[NOTE]
====
_sourceSurveyID_ and _surveyAuthority_ will use the following HDF5 encoding (variable length string per <<iho-s100,part=10c,table=1>>):
[source,c]
----
H5T_STRING {
...
The QualityOfBathymetryCoverage.01 instance group uses the same metadata attribute definitions as BathymetryCoverage instance groups (see <<tab-attributes-of-bathymetrycoverage-feature-instance-group>>). These attributes have the same meaning and permitted value domains as those defined for the BathymetryCoverage instance groups.
...
The values dataset for QualityOfBathymetryCoverage is a single two-dimensional array of unsigned integers (the same datatype and size as the “featureAttributeTableKey” field in featureAttributeTable — <<tab-attributes-of-values-group>>). The array must have the same dimensions as the values dataset in the BathymetryCoverage feature instance (<<subsec-BathymetryCoverage-feature-instance-group-values-dataset>>). The values dataset uses the Compound data type described in <<iho-s100,part=10c,table=18>> - Structure of values groups.
