...
=== Application Schema
The Application Schema Data Set Structure is shown in <<fig-data-set-structure-s102>> and <<fig-coverage-structure-of-s102>>. They show a number of classes specialized for use in {series}-{docnumber} and six sets of implementation classes. An actual data set of {series}-{docnumber} bathymetry data contains only the implementation classes. All of the required attributes from the other classes in the application schema are satisfied by statements within the Product Specification. This approach to producing the Application Schema results in a very simple structure for implementation.

[[fig-data-set-structure-s102]]
.Data Set Structure of {series}-{docnumber}
...
* An {series}-{docnumber} data set (*S102_DataSet*), which is inherited from *S100_DataSet*, references an {series}-{docnumber} Image and Gridded Data Collection (*S102_IGCollection*). In S-100 it is possible to have multiple collections but in {series}-{docnumber} only one is needed to hold the bathymetry coverage. The {series}-{docnumber} discovery metadata class (*S102_DiscoveryMetadata*) describes the metadata entities required for the identification of the entire data set. The required discovery metadata is implemented through the *S100_DatasetDiscoveryMetadata* class defined in <<iho-s100,part=17>>.

* An instance of an {series}-{docnumber} Image and Gridded Data Collection (*S102_IGCollection*) which is a subtype of *S100_IGCollection*, is described by a set of {series}-{docnumber} Collection Metadata (*S102_CollectionMetadata*). This relationship is 1 to 1 meaning that there is one set of collection metadata for each instance of *S102_IGCollection*. There is a large choice of metadata that may be used in an S-100 compliant data product. Only a small amount of this metadata is mandated by <<iso-19115-1>> for discovery. This edition of {series}-{docnumber} neither uses ISO metadata files nor extends S-100 generic metadata and therefore *S102_CollectionMetadata*, *S102_StructuralMetadata*, *S102_QualityMetadata*, and *S102_AcquisitionMetadata* are abstract classes as described in <<iho-s100,part=8,clause=8.5.3>>. This edition of {series}-{docnumber} uses the dataset metadata elements defined in <<iho-s100,part=17>> and <<iho-s100,part=10c>> with restrictions defined in this product specification. The metadata elements defined in <<iho-s100,part=17>> are encoded in a discovery block within the exchange catalogue (<<iho-s100,part=17,clause=17.4.4>> to <<iho-s100,part=17,clause=17.4.5>>), and the metadata elements defined in <<iho-s100,part=10c>> are encoded as attributes and datasets within the HDF5 file (<<iho-s100,part=10c,clause=10c.9.2>>). The conceptual structure of coverage features in an {series}-{docnumber} dataset is discussed further in <<subsec-application-schema-implementation-classes>>.

[[fig-coverage-structure-of-s102]]
.Coverage Structure of {series}-{docnumber}
...
The class *S102_QualityValues* is related to *QualityOfBathymetryCoverage* by a composition relationship in which an ordered sequence of _featureAttributeTableKey_ values provide for each cell references to metadata. The class *S102_QualityValues* inherits from S100_IF_Grid.

[level=7]
===== values

The attribute _values_ has the value type *_S102_QualityValueRecord_* which is a sequence of value items that shall assign values to the grid points. There is one attribute in the quality value record, _featureAttributeTableKey_, in the *S102_QualityValues* class.
...
The attribute _type_ shall identify the type of sequencing method that shall be used. A code list of scan types is provided in <<iho-s100,part=10c>>. Only the value "linear" shall be used in {series}-{docnumber}, which describes scanning row by row by column.
...
==== Introduction
The {series}-{docnumber} Feature Catalogue describes the feature types, attributes and attribute values which may be used in the product.

[[tsf]]The {series}-{docnumber} Feature Catalogue is available in an XML document which conforms to the S-100 XML Feature Catalogue Schema and can be downloaded from the IHO Geospatial Information Registry.[[ihoweb]]

==== Feature types
{series}-{docnumber} is a coverage feature product. *BathymetryCoverage* implements *S102_DepthCoverage* and includes *S102_BathymetryValues*. *QualityOfBathymetryCoverage* implements *S102_QualityCoverage* and includes *S102_QualityValues*.

===== Geographic
Geographic (geo) feature types form the principle content of the dataset and are fully defined by their associated attributes. In {series}-{docnumber}, *BathymetryCoverage* has been registered as a geographic feature type.

===== Meta
Meta feature types are allowed in the dataset and are fully defined by their associated attributes. In {series}-{docnumber}, *QualityOfBathymetryCoverage* has been registered as a meta feature type.

==== Feature relationship
{series}-{docnumber} does not use any feature relationships.

==== Attributes

===== Simple attributes

In {series}-{docnumber}, _depth_ and _uncertainty_ have been registered as simple attributes, type `<real>`; and _featureAttributeTableKey_ has been registered as a simple attribute, type `<integer>`. Simple attributes are defined in <<iho-s100,part=5,clause=5-4.2.3.3>>. 

===== Complex attributes
In {series}-{docnumber} there are currently no complex attributes defined.
...
The georeferencing for an {series}-{docnumber} Bathymetric Surface product shall be grid point based, referenced from the southwesternmost grid point. The reference position (grid origin) included in the metadata, precisely the attributes _gridOriginLatitude_ and _gridOriginLongitude_, shall be given in the coordinates used for the grid and shall contain sufficient digits of precision to locate the grid with accuracy no worse than a decimetre on the surface of the ellipsoid of rotation of the chosen horizontal datum.

[[fig-s102-cell-area-and-Bbox]]
.{series}-{docnumber} grid structure. Grid origin is the lower left grid point. Grid cells are centered around grid points. Bounding box is coincident with the outermost grid cell boundaries.
image::../images/figure-s102-cell-area-and-Bbox.png["{series}-{docnumber} grid components"]

==== Regular grid

===== {series}-{docnumber} coverages
The *BathymetryCoverage* contains _depth_ and, optionally, _uncertainty_. The general structure of each is defined in <<iho-s100,part=8>> as a georectified grid. Similarly, the *QualityOfBathymetryCoverage* contains _featureAttributeTableKey_.
...
.{series}-{docnumber} Grid point location
...
For *BathymetryCoverage.nn*, _depth_ and optional _uncertainty_, are stored in the same grid as members of a data compound. The units of the depth values are in metres. The vertical distance is from a given water level to the bottom. Drying heights (drying soundings) are indicated by a negative depth value. For *QualityOfBathymetryCoverage.01*, only _featureAttributeTableKey_ is stored in the grid.

The unknown state for depth is defined to be 1,000,000.0 (1.0e6).

The uncertainty values are expressed as positive quantities at each grid cell. As detailed in <<tab-elements-of-featureAttributeTable-compound-datatype>> and <<tab-codes-defining-how-bathy-depth-uncertainty-determined>> the uncertainty grid supports multiple definitions of vertical uncertainty. A recipient of an {series}-{docnumber} file can refer to the uncertainty definition in the Metadata to gain an understanding of how the uncertainty was computed.

The undetermined state for uncertainty is defined to be 1,000,000.0 (1.0e6).

The _featureAttributeTableKey_ values in *QualityOfBathymetryCoverage.01* are expressed as unsigned 32-bit integer at each grid cell. Each value corresponds to the identifier for a metadata record in a *featureAttributeTable* <<root-QualityOfBathymetryCoverage>>. 

The fill value for _featureAttributeTableKey_ is defined to be 0.

===== Extensions
In {series}-{docnumber} there are currently no extensions defined.

=== Multiple datasets
In order to facilitate the efficient processing of {series}-{docnumber} data, the geographic coverage of a given *maximum display Scale* may be split into multiple datasets.

=== Dataset rules
Each {series}-{docnumber} dataset must only have a single extent as it is a coverage feature. 
...
=== Geometry
{series}-{docnumber} regular gridded coverages are an implementation of S100_IF_GridCoverage <<iho-s100,part=8,clause=8.7>>. 
